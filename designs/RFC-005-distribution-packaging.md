...
- `[package.metadata.deb]` and `[package.metadata.generate-rpm]` sections are in
  `clients/rttx/Cargo.toml`. DEB and RPM packages include both the GUI and daemon binaries.
...
- [x] **RPM spec** — `packaging/rttx/rttx.spec` (hand-written); `[package.metadata.generate-rpm]` in `Cargo.toml` — PR #873
